...
Na Fase~1, a frequência $f_{n,1}^{opt}$ identificada pelo pico da FFT é comparada ao valor teórico $f_{n,1}^{anal}$ calculado pela Equação~\ref{eq:fn_cantilever} para a geometria e o material reais da régua. O critério de aprovação é:
\begin{equation}
    \varepsilon = \frac{\bigl|f_{n,1}^{opt} - f_{n,1}^{anal}\bigr|}{f_{n,1}^{anal}} \times 100\% < 5\%.
