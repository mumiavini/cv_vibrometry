...
\subsubsection{Iluminação e calibração espacial}

A iluminação é provida por painéis LED alimentados por fonte de corrente constante, eliminando a modulação de intensidade periódica associada ao controle por largura de pulso (PWM) presente em muitas luminárias convencionais. Variações temporais de intensidade luminosa podem ser interpretadas pelo algoritmo de fase como movimento aparente, gerando componentes espectrais espúrias \cite{robotcamera2025}. Os painéis são posicionados frontalmente e de forma difusa para minimizar reflexos especulares sobre a superfície da régua.
...
\subsubsection{Validação numérica do algoritmo}

Antes dos ensaios experimentais, o pipeline de filtros de Gabor é validado com dados sintéticos (Fase~1, CW~32--33). O sinal de deslocamento na ponta da viga é gerado numericamente como a resposta livre subamortecida (Equação~\ref{eq:resposta_subamortecida}) com $\omega_n$ e $\zeta$ calculados a partir das Equações~\ref{eq:fn_cantilever} e~\ref{eq:keq_meq}. Desse sinal, geram-se sequências de imagens sintéticas em que um marcador vertical se translada lateralmente conforme $x(t)$, reproduzindo o comportamento esperado no vídeo real. O algoritmo processa essas imagens e a frequência natural extraída pela FFT é comparada ao valor analítico de referência. A etapa é considerada aprovada quando o desvio relativo for inferior a 2\%, garantindo que discrepâncias observadas nos experimentos reais sejam atribuídas a efeitos físicos e não a erros de implementação.
